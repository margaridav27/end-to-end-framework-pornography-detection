...
def parse_model_filename(model_filename: str):
    ...
    model_name = (
        model_filename_split[0]
        if model_filename_split[1] == "freeze"
        else "_".join(model_filename_split[:2])
    )
...
def init_model(model_name: str, weights: str = None, freeze_layers: bool = False):
...
def train_model(
    model, dataloaders, dataset_sizes, optim, learning_rate, n_epochs, device, wandb_on
):
    """
    ...
    if optim == "adam":
        ...
        optimizer = torch.optim.SGD(params=params, lr=learning_rate, momentum=0.9)
